...
CODEX_AUTO_UPDATE_SKILLS = CLAUDE_AUTO_UPDATE_SKILLS.copy()
...
class TestProviderUpdateSkills:
    ...
    def test_update_state_and_locks_are_ignored(self, project_root: Path) -> None:
        ...
        for base_dir in (skills_dir, template_skills_dir):
            for skill_name, profile in WORKFLOW_EFFORT_PROFILES.items():
                ...
                assert frontmatter.get("effort") == expected_effort, (
                    f"{skill_file} should set Claude Code effort: {expected_effort}"
                )
        ...
        for folder in skill_folders:
            ...
            for href in link_re.findall(content):
                ...
                assert resolved.exists(), (
                    f"Skill '{folder}' links to missing bundled supporting file: {href}"
                )
            ...
            for script in scripts_dir.iterdir():
                ...
                if (
                    ...
                    assert os.access(script, os.X_OK), (
                        f"Script '{script}' is not executable. Run: chmod +x {script}"
                    )
