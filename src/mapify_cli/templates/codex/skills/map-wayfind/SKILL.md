...
- Parallelize ONLY independent researcher reads dispatched against research tickets. Keep charting interview, claiming, human grilling, resolving, and handoff strictly sequential.
...
5. **Kick off research.** For each `research` ticket, `claim_ticket` it FIRST — `resolve_ticket` requires the ticket to be claimed by your session, so an unclaimed dispatch-then-resolve returns `not_owner`. Serialize these claim mutations, then you MAY dispatch researcher subagents in parallel to gather findings; each writes to `resolutions/<ticket>.md`, after which you `resolve_ticket` it (research resolves are exempt from the one-per-session limit).
...
3. **Resolve by type.**
   ...
   - **3b. `prototype` / `grilling` (human-in-the-loop):** ask the human the ticket's question verbatim with a direct concise question (describe the cheap probe for prototype), then STOP and hand control back. Do not answer on the human's behalf. When the human replies, save their words VERBATIM to `resolutions/<ticket>.human.md` and register them:
