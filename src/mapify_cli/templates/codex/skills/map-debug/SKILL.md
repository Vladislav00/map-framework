...
- Start with `decomposer` so investigation, fix, and verification work are separated.
- Use `actor` for each investigation or fix subtask rather than a default agent.
...
```
1. DECOMPOSE → decomposer (break down debugging steps)
...
Before calling decomposer, gather context:

1. **Read error logs/stack traces** (if provided in $ARGUMENTS)
2. **Identify affected files**: Use rg through the command runner to find relevant code
...
spawn_agent(
  ...
  task_name="decompose_debugging_steps",
  message="<documents>
...
```
Derive each task name from the normalized debugging step id and attempt number
(`investigate_<step>_<attempt>`); never spawn a second agent with an existing
name. Use `followup_task` when continuing the same agent after feedback.

spawn_agent(
  agent_type="actor",
  task_name=INVESTIGATION_TASK_NAME,
  message="Investigate this debugging step:
...
Read files and use rg through the command runner to analyze code. Do NOT make changes yet."
)
```
...
spawn_agent(
  agent_type="actor",
  task_name=FIX_TASK_NAME,
  message="Implement a fix for this issue:

**Issue:** [from investigation]
**Root Cause:** [identified root cause]

Apply the fix directly with `apply_patch`.
...
spawn_agent(
  ...
  task_name=PREDICTOR_TASK_NAME,
  message="Analyze the impact of this debugging fix:
...
spawn_agent(
  ...
  message="Evaluate this debugging fix:
...
- Use spawn_agent to call the configured specialized subagents in the sequence above.
...
2. spawn_agent(agent_type="decomposer") → get investigation + fix steps
3. For investigation steps: spawn_agent(agent_type="actor") to analyze
