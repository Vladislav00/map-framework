...
When its input is clearly a PRD, product brief, feature brief, or requirements document, `/map-plan` first offers an **optional** `/map-prd-review` preflight. You can also invoke `/map-prd-review <path-or-inline-text>` directly. It scores 13 applicable dimensions from 0–10 (the overall score is their equal-weight mean), reports evidence-backed strengths and severity-ranked weaknesses/risks, and lists feature-specific uncovered edge cases. `ready_for_plan` requires a score of at least 8.0 with no critical or major findings; non-applicable dimensions are recorded as `null` and excluded from the mean.

A non-ready review does not force a stop. `/map-plan` shows the score and material gaps, then asks whether to continue planning anyway or stop and revise the PRD. Either explicit choice is persisted in `.map/<branch>/prd-review.json`; choosing `proceed_anyway` carries every finding, blocking question, uncovered edge case, suggested revision, and route recommendation into the spec's Open Questions / Risks as deduplicated `PRD-GAP-N` entries. Dependent subtasks cite `Provisional — blocked on PRD-GAP-N` in their descriptions and validation criteria. The override records accepted uncertainty—it does not change `ready_for_plan` to true. Resume detection runs before this offer, so resuming an existing matching plan preserves its prior PRD review and planning decision.

The durable review artifacts are `.map/<branch>/prd-review.json` and `.map/<branch>/prd-review.md`. Claude Code invokes `/map-prd-review`; Codex invokes `$map-prd-review`.
...
Once a plan reaches `ready` (task_plan + blueprint both present, the Step 7 boundary), `record_plan_artifacts` creates a `plan_approval` checkpoint — a durable approval hold that surfaces its hold id and blocks continuation until a human decides it. `/map-efficient`'s `INIT_STATE` phase and `/map-task`'s preflight both check for it and stop to ask interactively before execution starts; `/map-auto` auto-approves it instead (see below).
...
Before launching Monitor, Predictor, Evaluator, and the two role reviewers, `/map-review` now runs `python3 .map/scripts/map_step_runner.py build_review_prompts` to assemble the fan-out prompts from the persisted bundle, review preferences, and raw `git diff`. Review prompts are **not truncated**: every reviewer receives the whole bundle, the preferences and the full diff. `MAP_REVIEW_PROMPT_BUDGET_TOKENS` is still echoed in the helper's output as `budget_tokens`, but it clips nothing — no `Review Prompt Budget` diagnostic is inserted and `clipped_sections` is always empty. When a review prompt outgrows the context window, that is an explicit operator decision (`/compact`, or splitting the change), never a silent drop of evidence.

Two role reviewers run in that same fan-out by default — no flag, no opt-in. `user_experience` asks one question: did the change make the ALREADY SHIPPED functionality harder or less convenient (extra mandatory steps on the old path, confusable flags such as `--release` vs `--from-release`, an explicitly-supplied value silently overridden instead of rejected), judged both as a human in a terminal and as a CI script. `maintainer` asks whether the next person can extend this without re-reading every line: branch-scoped plan/tracker IDs left in comments, implementation vocabulary in help/error text, copy-paste, a decision computed in two places, version checks named by release number instead of capability, embedded shell/SQL/YAML in string literals. Every role finding must arrive with all five contract parts — `problem` (with `file:line`), `current_code` verbatim, `proposed_code` applicable as a patch, a MEASURABLE `why_better`, and its `cost` — or it is dropped as `contract_incomplete`, reported and never counted toward the verdict. Where it is dropped depends on the path: on the normal fan-out the ledger tombstones it and names it in `not_verified`, and when its severity is above `minor` it also sets `escalation_required`, so a CRITICAL or IMPORTANT claim lost to a missing `cost` costs the run its PROCEED instead of vanishing; under `--adversarial` the aggregator removes it before the ledger and lists it under `contract_incomplete` in the report. Under `--adversarial` both roles join the reviewer set (five reviewers; `--quick` drops only the Edge Case Hunter). To build just their prompts: `python3 .map/scripts/map_step_runner.py build_role_review_prompts [--roles user_experience,maintainer]`.

At closeout `/map-review` computes its verdict instead of choosing one. Each reviewer's JSON envelope is written to `.map/<branch>/review-agent-<role>.json`, and `python3 .map/scripts/map_step_runner.py write_review_verdict_ledger --monitor-file … --predictor-file … --evaluator-file … --user-experience-file … --maintainer-file …` normalizes them into `.map/<branch>/review-verdict-ledger.json` plus a human-readable `.md`. The decision table counts every finding whose status is `active` or `downgraded`. Only a finding proven `minor` may be tombstoned, by any route — `needs_investigation` sits inside that floor because it means "severity not established", not "low severity". A finding that lacks `reach_evidence`, that a reviewer asserts predates the PR, or that an operator contests on a checkable channel is downgraded to `needs_investigation` and still counted, so no metadata gap, self-attested claim or unverified objection can erase a blocking finding from the gate. Missing, unreadable or malformed reviewer output is itself a finding, so an empty registry reads as "the review was not observed" rather than as a pass.
...
This helper writes nothing to `.map/<branch>/token_budget.json` — that file records `/map-efficient` Actor `<map_context>` decisions only. A suspicious review result is therefore never explained by clipping: every reviewer saw the whole bundle and the full diff.
