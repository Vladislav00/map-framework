...
- **Context Budgeting & Compression**: MAP exposes compression policies and thresholds and treats budget control as a first-class workflow concern. Generated Actor `<map_context>` blocks are capped by a deterministic estimated-token budget before prompt injection so long plans or raw diffs cannot silently crowd out current-subtask and dependency context, and that path appends before/after estimates and clipped section labels to `.map/<branch>/token_budget.json` so users can diagnose missing-context reports without transcript inspection. `/map-review` reviewer prompts are deliberately **exempt**: their truncation path was removed, so a reviewer always sees the whole bundle and the full diff, and an oversized prompt is an explicit operator decision rather than a silent clip.
...
- **PRD Readiness Review** (`/map-prd-review`, `$map-prd-review`): an optional, pre-planning review scores the equal-weight mean of 13 applicable dimensions from 0–10 and persists `.map/<branch>/prd-review.{json,md}` with evidence-backed strengths, severity-ranked weaknesses/risks, uncovered edge cases, blocking questions, and revisions. `ready_for_plan` requires score ≥ 8.0 with no critical/major findings. `/map-plan` performs resume detection first; only a fresh plan with clear PRD input gets the optional offer, preserving any recorded decision on resumed plans. A non-ready verdict pauses for an explicit `proceed_anyway` or `stop_for_revision` choice recorded by `record_prd_review_decision()`. Proceeding preserves `ready_for_plan: false`, carries every deduplicated review gap into the spec as `PRD-GAP-N` entries, and annotates dependent subtask descriptions and validation criteria `Provisional — blocked on PRD-GAP-N` rather than inventing a new blueprint field or silently treating uncertainty as resolved.
...
- **Token Budget Decisions**: `.map/<branch>/token_budget.json` records active prompt-path budget decisions from Actor context generation only; it does not log dormant REGISTRY/FOCUS experiments, and it no longer receives review-prompt entries (that builder stopped clipping). Each entry names the prompt path, configured budget, estimated tokens before/after enforcement, clipped sections, and source artifacts.
...
**Review-Bundle-First Context:** `/map-review` persists the durable review bundle (`.map/<branch>/review-bundle.json` / `.map/<branch>/review-bundle.md`) via `create_review_bundle()` before launching reviewer agents. It then calls `build_review_prompts` to create separate Monitor, Predictor, Evaluator and role-reviewer prompts. The helper keeps the bundle as primary context — containing spec, task plan, test contracts, verification summary, code-review history, acceptance-tag coverage, and prior-stage consumption status — and preserves reviewer instructions and expected output schemas. Review prompts are **not clipped**: the truncation path was removed deliberately, so every reviewer sees the whole bundle, the preferences and the full diff, `clipped_sections` is always empty, and no `Review Prompt Budget` diagnostic is emitted. `MAP_REVIEW_PROMPT_BUDGET_TOKENS` survives only as a reported `budget_tokens` value; deciding what to do about an oversized prompt (`/compact`, or splitting the change) is the operator's, not a silent drop of evidence. When `minimality` is not `off`, the helper also emits an advisory complexity-only what-to-delete lens (`delete`/`stdlib`/`native`/`yagni`/`shrink` plus `net: -N`) that is never used as a verdict gate or Actor retry input. When detached preparation is unavailable, the review still proceeds from the persisted bundle. Bundle generation always updates `artifact_manifest.json["stages"]["review"]`.
...
Their output is governed by a **five-part contract** — `problem` (one line + `file:line`), `current_code` (verbatim), `proposed_code` (applicable as a patch), `why_better` (measurable delta; bare "cleaner" is rejected), `cost` (downside or `none`) — declared once as `ROLE_FINDING_SCHEMA` and reused as the `AGENT_OUTPUT_SCHEMAS` skeleton for both roles, so the rendered prompt, the truncation gate (`--agent user_experience` / `--agent maintainer`) and the JSON retry prompt cannot disagree. `role_finding_contract_gaps()` is the single enforcement point, and where an incomplete finding lands depends on which path ran. On the NORMAL fan-out the ledger tombstones it with `transition_reason: contract_incomplete` and names it in `not_verified`; this is the one row tombstoned at ANY severity, so above `minor` it additionally appends an `escalation_reason` and `escalation_required` is set — PROCEED becomes unavailable and a human rules on it. Under `--adversarial` the finding never reaches the ledger: `aggregate_adversarial_findings` removes it first and reports it under `contract_incomplete` in the aggregation report. Either way it never gates the change and never silently disappears. That asymmetry is deliberate: an unfinished remark must not block a merge on its own, but a reviewer that stopped halfway must not read as "nothing found", and a dropped CRITICAL must not buy a clean pass.
