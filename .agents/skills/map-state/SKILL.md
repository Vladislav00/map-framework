...
### Initialization
```bash
bash .agents/skills/map-state/scripts/init-session.sh
```
...
2. decomposer populates phases from blueprint
...
- **NEVER** write `task_plan` `**Status:**` from any agent other than Monitor. decomposer creates the plan; Monitor owns every subsequent status transition (see Single-Writer Governance). An agent that needs a status change must surface it, not write it.
