...
Ask the user directly which bump to perform: PATCH for fixes, MINOR for
backward-compatible features, MAJOR for breaking changes, or an explicit
`X.Y.Z`. Do not choose on the user's behalf and do not continue until they
answer.
...
```bash
LAST_TAG=$(git tag --sort=-version:refname | sed -n '1p')
...
Ask the user directly for explicit confirmation before pushing. State that
`$LAST_TAG` to `origin/main` will trigger GitHub Actions, publish to PyPI, and
create a public GitHub release. Accept only an unambiguous YES; NO aborts, and
REVIEW shows the commit, tag, and CHANGELOG before asking again.
...
- **Direct user confirmation** — explicit confirmation for IRREVERSIBLE operations
