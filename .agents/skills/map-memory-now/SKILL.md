...
**Requires:**
- `codex` CLI (finalization uses non-interactive `codex exec` with structured output)
...
n = finalize_dirty(None, ".", provider="codex")
print(f"map-memory-now: finalized {n} digest(s)")
PY
```

`finalize_dirty(None, ".", provider="codex")` is the `--finalize-all` sweep: it finalizes every
...
- **`codex: command not found`:** install the Codex CLI or put it on `PATH`.
  The digest remains unfinalized and can be retried after installation.
- **Digest not written / scratch still present:** finalize is best-effort and atomic —
  on a `codex exec` timeout or error the scratch is left unfinalized (no partial digest)
...
- **No new CLI subcommand needed:** `finalize_dirty(None, project_dir, provider="codex")` IS the
