...
```bash
SUBTASK_ID=$(printf '%s' "$ARGUMENTS" | grep -m1 -oE 'ST-[0-9]+')
...
- **RESEARCH (2.2)** — Required persisted research artifact; researcher is conditional for broad/high-risk discovery.
...
4. **Suggest next subtask.** Ask the user directly whether to execute
   `$map-task ${SUGGESTED}`, start `$map-tdd ${SUGGESTED}`, or stop for now.
   Do not start the next subtask until the user answers.
